...
```bash
git clone https://github.com/kizuna-intelligence/Irodori-TTS-Lite.git
cd Irodori-TTS-Lite
git lfs pull                       # 重みは LFS で配信されます

pip install -e .                   # 本パッケージ
pip install pyopenjtalk            # 例 (run_tts.py) でテキスト → 秒数の自動推定に使用
...
`--no-ref` は話者性を重みに焼き込んだチェックポイント用フラグ。
...
python tools/measure_peak_memory.py \
    --mode int4 \
    --checkpoint weights/dit_int4.safetensors \
    --no-ref --codec-device cpu --json

# DACVAE もそのまま GPU で int4 化（VRAM 約 1 GB でフル推論）
python tools/measure_peak_memory.py \
    --mode int4 \
    --checkpoint weights/dit_int4.safetensors \
    --no-ref --codec-int4 --json
```

未量子化との比較は `--mode bf16` / `--mode fp32` に切り替え + 対応する未量子化 checkpoint を渡してください。
